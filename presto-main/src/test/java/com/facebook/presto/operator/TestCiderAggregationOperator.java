...
public class TestCiderAggregationOperator
{
    ...
    @Test
    public void testCiderAggregationFallback()
    {
        ...
        OperatorFactory operatorFactory = new CiderAggregationOperator.CiderAggregationOperatorFactory(
                0,
                new PlanNodeId("test"),
                AggregationNode.Step.SINGLE,  // cider only consider Partial for now, so Single will fallback
                ...
                false,
                null); // node is used by Cider plan only. so it's ok to pass a null.
    ...
    @Test
    public void testCiderAggregation()
    {
        InternalAggregationFunction countVarcharColumn = getAggregation("count", VARCHAR);
        InternalAggregationFunction maxVarcharColumn = getAggregation("max", VARCHAR);
        List<Page> input = rowPagesBuilder(VARCHAR, BIGINT, VARCHAR, BIGINT, REAL, DOUBLE, VARCHAR)
